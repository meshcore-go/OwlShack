...
type RepeaterNeighbor struct {
	PubkeyPrefix string `json:"pubkeyPrefix"`
	SecsAgo      uint32 `json:"secsAgo"`
	SNR          int    `json:"snr"`
}

type RepeaterNeighbors struct {
	TotalCount   int                `json:"totalCount"`
	ResultsCount int                `json:"resultsCount"`
	Neighbors    []RepeaterNeighbor `json:"neighbors"`
}

type RepeaterOwnerInfo struct {
...
type RepeaterAccessList struct {
...
type RepeaterManager struct {
	...
	mu       sync.Mutex
	sessions map[string]*RepeaterSession
...
func NewRepeaterManager(n *node.Node, st *store.Store, log *slog.Logger) *RepeaterManager {
	return &RepeaterManager{
		node:       n,
		store:      st,
		log:        log,
		sessions:   make(map[string]*RepeaterSession),
		pending:    make(map[uint32]*pendingRequest),
		cliPending: make(map[string]chan string),
	}
}

func (rm *RepeaterManager) persistOutPath(pubkey []byte, path []byte, hashSize uint8) {
...
func (rm *RepeaterManager) Session(pubkeyHex string) *RepeaterSession {
	rm.mu.Lock()
	defer rm.mu.Unlock()
	return rm.sessions[pubkeyHex]
}

func (rm *RepeaterManager) Logout(pubkeyHex string) {
	rm.mu.Lock()
	defer rm.mu.Unlock()
...
func (rm *RepeaterManager) GetPeerPath(pubkeyHex string) (*PeerPathInfo, error) {
...
func (rm *RepeaterManager) ResetPeerPath(pubkeyHex string) error {
	pubkeyBytes, err := hex.DecodeString(pubkeyHex)
	if err != nil {
...
func (rm *RepeaterManager) SetPeerPath(pubkeyHex, pathHex string, pathHashSize int) error {
	pubkeyBytes, err := hex.DecodeString(pubkeyHex)
	if err != nil {
		return fmt.Errorf("invalid pubkey hex: %w", err)
	}
...
func (rm *RepeaterManager) SendLogin(pubkeyHex, password string, timeout time.Duration) (*LoginResult, error) {
	pubkeyBytes, err := hex.DecodeString(pubkeyHex)
	if err != nil {
		return nil, fmt.Errorf("invalid pubkey hex: %w", err)
	}
	...
	case data := <-resultCh:
		...
		rm.mu.Lock()
		rm.sessions[pubkeyHex] = &RepeaterSession{
			PubKeyHex:       pubkeyHex,
			IsAdmin:         isAdmin,
			LoggedInAt:      time.Now(),
			sharedSecret:    sharedSecret,
...
func (rm *RepeaterManager) sendBinaryRequest(pubkeyHex string, body []byte, timeout time.Duration, label string) ([]byte, error) {
	pubkeyBytes, err := hex.DecodeString(pubkeyHex)
	if err != nil {
		return nil, fmt.Errorf("invalid pubkey hex: %w", err)
	}
	...
	resultCh := make(chan []byte, 1)
	rm.pendingMu.Lock()
	rm.pending[tag] = &pendingRequest{ch: resultCh, created: time.Now()}
	rm.pendingMu.Unlock()
	...
	rm.log.Debug(label+" req sent", "peer", pubkeyHex[:12], "tag", fmt.Sprintf("%08x", tag))
...
func (rm *RepeaterManager) SendStatusReq(pubkeyHex string, timeout time.Duration) (*RepeaterStatus, error) {
	body := make([]byte, 5)
...
func (rm *RepeaterManager) SendNeighborsReq(pubkeyHex string, count uint8, offset uint16, timeout time.Duration) (*RepeaterNeighbors, error) {
...
func (rm *RepeaterManager) SendOwnerInfoReq(pubkeyHex string, timeout time.Duration) (*RepeaterOwnerInfo, error) {
	body := make([]byte, 5)
...
func (rm *RepeaterManager) SendAccessListReq(pubkeyHex string, timeout time.Duration) (*RepeaterAccessList, error) {
...
func (rm *RepeaterManager) SetAccessPerm(pubkeyHex, targetPubkeyHex string, perms uint8, timeout time.Duration) error {
...
func (rm *RepeaterManager) SendTelemetryReq(pubkeyHex string, timeout time.Duration) (*RepeaterTelemetry, error) {
	// payload: type(1) inverse_perm_mask(1) reserved(3) random(4)
	// inverse mask 0x00 = request all available permissions
	body := make([]byte, 9)
...
func (rm *RepeaterManager) SendCLI(pubkeyHex, command string, timeout time.Duration) (string, error) {
	pubkeyBytes, err := hex.DecodeString(pubkeyHex)
	if err != nil {
		return "", fmt.Errorf("invalid pubkey hex: %w", err)
	}
...
func (rm *RepeaterManager) HandlePathPacket(pkt *meshcore.Packet) bool {
	...
	rm.mu.Lock()
	sessions := make([]*RepeaterSession, 0, len(rm.sessions))
...
func (rm *RepeaterManager) HandleResponsePacket(pkt *meshcore.Packet) {
...
func (rm *RepeaterManager) HandleCLIResponse(senderPubKey [32]byte, text string) {
...
func (rm *RepeaterManager) HandleTextPacket(pkt *meshcore.Packet) bool {
...
func parseRepeaterNeighbors(data []byte, prefixLen int) (*RepeaterNeighbors, error) {
	if len(data) < 4 {
		return nil, fmt.Errorf("neighbors data too short: got %d", len(data))
	}
	out := &RepeaterNeighbors{
	...
	for i := 0; i < out.ResultsCount; i++ {
		...
		out.Neighbors = append(out.Neighbors, RepeaterNeighbor{
...
func parseRepeaterAccessList(data []byte) *RepeaterAccessList {
	out := &RepeaterAccessList{Entries: []AccessListEntry{}}
...
func parseRepeaterOwnerInfo(data []byte) *RepeaterOwnerInfo {
	text := strings.TrimRight(string(data), "\x00")
	parts := strings.SplitN(text, "\n", 3)
	info := &RepeaterOwnerInfo{}
	if len(parts) > 0 {
...
func parseRepeaterTelemetry(data []byte) (*RepeaterTelemetry, error) {
	out := &RepeaterTelemetry{
...
func lppTypeMeta(typ byte) (name, unit string) {
	switch typ {
...
func parseRepeaterStatus(data []byte) (*RepeaterStatus, error) {
	if len(data) < 52 {
